    ...
    <v-card class="text-center edit-board-card">
      ...
      <v-form class="p-6" v-model="form">
        ...
        <v-row class="d-flex align-center justify-center">
          ...
          <v-col cols="12" sm="6" md="4">
            <v-btn :loading="loading" block rounded="xs" size="x-large"  :disabled="this.pumperCode.length === 0 || this.pumperName.length === 0 || this.pulseDuration.length === 0" @click="editBoard">Submit</v-btn>
          </v-col>
        </v-row>
      </v-form>
    </v-card>
